...
func doTestMultiSitesMainLangInRoot(t *testing.T, defaultInSubDir bool) {
	...
	siteConfig := testSiteConfig{Fs: fsintra.NewMemMapFs(), DefaultContentLanguage: "fr", DefaultContentLanguageInSubdir: defaultInSubDir}
...
func doTestMultiSitesBuild(t *testing.T, configTemplate, configSuffix string) {
	siteConfig := testSiteConfig{Fs: fsintra.NewMemMapFs(), DefaultContentLanguage: "fr", DefaultContentLanguageInSubdir: true}
...
`
	...
	require.NoError(t, sites.Cfg.(*helpers.Language).Cfg.(*configurator.Configurator).ReadInConfig())
...
func TestChangeDefaultLanguage(t *testing.T) {
	...
	require.NoError(t, th.Cfg.(*helpers.Language).Cfg.(*configurator.Configurator).ReadInConfig())
	err = sites.Build(BuildCfg{CreateSitesFromConfig: true})
...
`)},
	...
	for _, s := range sources {
		if err := fsintra.WriteFile(mf, filepath.Join("content", s.Name), s.Content, 0755); err != nil {
...
func writeSource(t testing.TB, fs *geanfs.Fs, filename, content string) {
	writeToFs(t, fs.Source, filename, content)
}

func writeToFs(t testing.TB, fs fsintra.Fs, filename, content string) {
	if err := fsintra.WriteFile(fs, filepath.FromSlash(filename), []byte(content), 0755); err != nil {
		t.Fatalf("Failed to write file: %s", err)
	}
}

func readDestination(t testing.TB, fs *geanfs.Fs, filename string) string {
	return readFileFromFs(t, fs.Destination, filename)
}

func destinationExists(fs *geanfs.Fs, filename string) bool {
...
func readSource(t *testing.T, fs *geanfs.Fs, filename string) string {
	return readFileFromFs(t, fs.Source, filename)
}

func readFileFromFs(t testing.TB, fs fsintra.Fs, filename string) string {
...
func writeNewContentFile(t *testing.T, fs *geanfs.Fs, title, date, filename string, weight int) {
