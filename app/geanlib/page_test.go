...
func TestDifferentFrontMatterVarTypes(t *testing.T) {
	...
	if assist.ToStringMap(param)["a_key"] != "a_value" {
		t.Errorf("frontmatter not handling values inside a table correctly should be %s, got: %s", "a_value", assist.ToStringMap(page.Params["a_table"])["a_key"])
	}
}
