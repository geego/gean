...
type Page struct {
	*pageInit
	...
	renderingConfig *helpers.Markdown
...
func (p *Page) Param(key interface{}) (interface{}, error) {
	keyStr, err := assist.ToStringE(key)
	if err != nil {
		return nil, err
	}
...
func traverse(keys []string, m map[string]interface{}) interface{} {
	...
	return traverse(rest, assist.ToStringMap(result))
}
...
func (p *Page) getRenderingConfig() *helpers.Markdown {
	p.renderingConfigInit.Do(func() {
		p.renderingConfig = p.s.ContentSpec.NewMarkdown()

		if p.Language() == nil {
			panic(fmt.Sprintf("nil language for %s with source lang %s", p.BaseFileName(), p.lang))
		}

		bfParam := p.GetParam("markdown")
		if bfParam == nil {
			return
		}

		pageParam := assist.ToStringMap(bfParam)
...
func (p *Page) update(f interface{}) error {
	...
	for k, v := range m {
		...
		switch loki {
		case "title":
			p.Title = assist.ToString(v)
			p.Params[loki] = p.Title
		case "linktitle":
			p.linkTitle = assist.ToString(v)
			p.Params[loki] = p.linkTitle
		case "description":
			p.Description = assist.ToString(v)
			p.Params[loki] = p.Description
		case "slug":
			p.Slug = assist.ToString(v)
			p.Params[loki] = p.Slug
		case "url":
			if url := assist.ToString(v); strings.HasPrefix(url, "http://") || strings.HasPrefix(url, "https://") {
				return fmt.Errorf("Only relative URLs are supported, %v provided", url)
			}
			p.URLPath.URL = assist.ToString(v)
			p.Params[loki] = p.URLPath.URL
		case "type":
			p.contentType = assist.ToString(v)
			p.Params[loki] = p.contentType
		case "extension", "ext":
			p.extension = assist.ToString(v)
			p.Params[loki] = p.extension
		case "keywords":
			p.Keywords = assist.ToStringSlice(v)
			p.Params[loki] = p.Keywords
		case "date":
			p.Date, err = assist.ToTimeE(v)
			if err != nil {
				p.s.Log.ERROR.Printf("Failed to parse date '%v' in page %s", v, p.File.Path())
			}
			p.Params[loki] = p.Date
		case "lastmod":
			p.Lastmod, err = assist.ToTimeE(v)
			if err != nil {
				p.s.Log.ERROR.Printf("Failed to parse lastmod '%v' in page %s", v, p.File.Path())
			}
		case "modified":
			vv, err := assist.ToTimeE(v)
			if err == nil {
				p.Params[loki] = vv
				modified = vv
			} else {
				p.Params[loki] = assist.ToString(v)
			}
		case "outputs":
			o := assist.ToStringSlice(v)
			if len(o) > 0 {
		...
		case "publishdate", "pubdate":
			p.PublishDate, err = assist.ToTimeE(v)
			if err != nil {
		...
		case "layout":
			p.Layout = assist.ToString(v)
			p.Params[loki] = p.Layout
		case "markup":
			p.Markup = assist.ToString(v)
			p.Params[loki] = p.Markup
		case "weight":
			p.Weight = assist.ToInt(v)
			p.Params[loki] = p.Weight
		case "aliases":
			p.Aliases = assist.ToStringSlice(v)
		...
		case "status":
			p.Status = assist.ToString(v)
			p.Params[loki] = p.Status
		case "sitemap":
			p.Sitemap = parseSitemap(assist.ToStringMap(v))
			p.Params[loki] = p.Sitemap
		case "iscjklanguage":
			isCJKLanguage = new(bool)
			*isCJKLanguage = assist.ToBool(v)
		case "translationkey":
			p.translationKey = assist.ToString(v)
...
func (p *Page) getParam(key string, stringToLower bool) interface{} {
	...
	case string:
		...
		return val
	case int64, int32, int16, int8, int:
		return assist.ToInt(v)
	case float64, float32:
		return assist.ToFloat64(v)
	case time.Time:
		return val
	case []string:
		if stringToLower {
...
func (p *Page) Menus() PageMenus {
	p.pageMenusInit.Do(func() {
		...
		if ms, ok := p.Params["menu"]; ok {
			...
			mnames, err := assist.ToStringSliceE(ms)
			...
			menus, err := assist.ToStringMapE(ms)
			...
			for name, menu := range menus {
				...
				if menu != nil {
					...
					ime, err := assist.ToStringMapE(menu)
					if err != nil {
