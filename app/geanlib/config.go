...
// a set of defaults.
func LoadConfig(fs fsintra.Fs, relativeSourcePath, configFilename string) (*configurator.Configurator, error) {
	v := configurator.New()
	v.SetFs(fs)
	...
	err := v.ReadInConfig()
	if err != nil {
		if _, ok := err.(configurator.ConfigParseError); ok {
			return nil, err
		}
...
func loadDefaultSettingsFor(v *configurator.Configurator) error {
	...
	v.SetDefault("markdown", c.NewMarkdown())
