...
func (m *MenuEntry) marshallMap(ime map[string]interface{}) {
	for k, v := range ime {
		...
		switch loki {
		case "url":
			m.URL = assist.ToString(v)
		case "weight":
			m.Weight = assist.ToInt(v)
		case "name":
			m.Name = assist.ToString(v)
		case "pre":
			m.Pre = template.HTML(assist.ToString(v))
		case "post":
			m.Post = template.HTML(assist.ToString(v))
		case "identifier":
			m.Identifier = assist.ToString(v)
		case "parent":
			m.Parent = assist.ToString(v)
		}
	}
}
