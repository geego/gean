...
func NewDirs(fs *geanfs.Fs, cfg config.Provider, logger *notepad.Notepad) (*Dirs, error) {
...
func (d *Dirs) CreateStaticFs() (fsintra.Fs, error) {
	var (
...
func (d *Dirs) createOverlayFs(absPaths []string) fsintra.Fs {
	source := d.pathSpec.Fs.Source

	if len(absPaths) == 1 {
		return fsintra.NewReadOnlyFs(fsintra.NewBasePathFs(source, absPaths[0]))
	}

	base := fsintra.NewReadOnlyFs(fsintra.NewBasePathFs(source, absPaths[0]))
	overlay := d.createOverlayFs(absPaths[1:])

	return fsintra.NewCopyOnWriteFs(base, overlay)
}
