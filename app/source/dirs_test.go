...
func TestStaticDirs(t *testing.T) {
	...
	tests := []struct {
		setup    func(cfg config.Provider, fs *geanfs.Fs) config.Provider
		expected []string
	}{

		{func(cfg config.Provider, fs *geanfs.Fs) config.Provider {
...
func readFileFromFs(t testing.TB, fs fsintra.Fs, filename string) string {
	...
	if err != nil {
		fsintra.Walk(fs, "", func(path string, info os.FileInfo, err error) error {
