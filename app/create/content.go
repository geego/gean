...
func NewContent(
	ps *helpers.PathSpec,
	siteFactory func(filename string, siteUsed bool) (*geanlib.Site, error), kind, targetPath string) error {
	ext := helpers.Ext(targetPath)

	notepad.INFO.Printf("attempting to create %q of %q of ext %q", targetPath, kind, ext)
	...
	notepad.FEEDBACK.Println(contentPath, "created")

	editor := s.Cfg.GetString("newContentEditor")
	if editor != "" {
		notepad.FEEDBACK.Printf("Editing %s with %q ...\n", targetPath, editor)
...
func findArchetype(ps *helpers.PathSpec, kind, ext string) (outpath string) {
	...
	if ps.Cfg.GetString("theme") != "" {
		...
		if _, err := ps.Fs.Source.Stat(themeDir); os.IsNotExist(err) {
			notepad.ERROR.Printf("Unable to find archetypes directory for theme %q at %q", ps.Cfg.GetString("theme"), themeDir)
		} else {
	...
	for _, x := range search {
		...
		for _, p := range pathsToCheck {
			...
			notepad.DEBUG.Println("checking", curpath, "for archetypes")
			if exists, _ := helpers.Exists(curpath, ps.Fs.Source); exists {
				notepad.INFO.Println("curpath: " + curpath)
				return curpath
			}
		}
	}
