...
func (ns *Namespace) Contains(s, substr interface{}) (bool, error) {
	...
	su, err := assist.ToStringE(substr)
	if err != nil {
		return false, err
	}
...
func (ns *Namespace) HasPrefix(s, prefix interface{}) (bool, error) {
	...
	sx, err := assist.ToStringE(prefix)
	if err != nil {
		return false, err
	}
...
func (ns *Namespace) HasSuffix(s, suffix interface{}) (bool, error) {
	...
	sx, err := assist.ToStringE(suffix)
	if err != nil {
		return false, err
	}
...
func (ns *Namespace) Replace(s, old, new interface{}) (string, error) {
	...
	so, err := assist.ToStringE(old)
	if err != nil {
		return "", err
	}

	sn, err := assist.ToStringE(new)
	if err != nil {
		return "", err
	}
...
func (ns *Namespace) SliceString(a interface{}, startEnd ...interface{}) (string, error) {
	...
	if argNum > 0 {
		if argStart, err = assist.ToIntE(startEnd[0]); err != nil {
			return "", errors.New("start argument must be integer")
		}
	}
	if argNum > 1 {
		if argEnd, err = assist.ToIntE(startEnd[1]); err != nil {
...
func (ns *Namespace) Substr(a interface{}, nums ...interface{}) (string, error) {
	...
	case 1:
		if start, err = assist.ToIntE(nums[0]); err != nil {
			return "", errors.New("start argument must be integer")
		}
		length = len(asRunes)
	case 2:
		...
		if length, err = assist.ToIntE(nums[1]); err != nil {
...
func (ns *Namespace) Trim(s, cutset interface{}) (string, error) {
	...
	sc, err := assist.ToStringE(cutset)
	if err != nil {
		return "", err
	}
