...
func (t *templateHandler) addTemplateFile(name, baseTemplatePath, path string) error {
	...
	getTemplate := func(filename string) (string, error) {
		b, err := fsintra.ReadFile(t.Fs.Source, filename)
		if err != nil {
			return "", err
		}
		return string(b), nil
	...
	case ".amber":
		...
		b, err := fsintra.ReadFile(t.Fs.Source, path)
	...
	case ".ace":
		...
		innerContent, err := fsintra.ReadFile(t.Fs.Source, path)
		...
		if baseTemplatePath != "" {
			baseContent, err = fsintra.ReadFile(t.Fs.Source, baseTemplatePath)
			if err != nil {
				return err
			}
		}
