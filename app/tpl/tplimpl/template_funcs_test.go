...
var (
	logger = notepad.NewNotepad(notepad.LevelFatal, notepad.LevelFatal, os.Stdout, ioutil.Discard, "", log.Ldate|log.Ltime)
)
...
func TestTemplateFuncsExamples(t *testing.T) {
	...
	fsintra.WriteFile(fs.Source, filepath.Join(workingDir, "README.txt"), []byte("Hugo Rocks!"), 0755)
...
func TestPartialCached(t *testing.T) {
	...
	config := newDepsConfig(configurator.New())
...
func newTestFuncster() *templateFuncster {
	return newTestFuncsterWithConfigurator(configurator.New())
}

func newTestFuncsterWithConfigurator(v *configurator.Configurator) *templateFuncster {
