...
func NewContentSpec(cfg config.Provider) (*ContentSpec, error) {
	spec := &ContentSpec{
		markdown:                   cfg.GetStringMap("markdown"),
	...
	if cfg.GetBool("pygmentsUseClassic") {
		if !hasPygments() {
			notepad.WARN.Println("Highlighting with pygmentsUseClassic set requires Pygments to be installed and in the path")
		} else {
			useClassic = true
		}
	}
...
// Markdown holds configuration values for Markdown rendering.
...
// NewMarkdown creates a new Markdown filled with site config or some sane defaults.
func (c ContentSpec) NewMarkdown() *Markdown {
	...
	if c.markdown != nil {
		for k, v := range c.markdown {
			siteConfig[k] = v
		}
	}

	combinedConfig := &Markdown{}
	if err := mapstructure.Decode(siteConfig, combinedConfig); err != nil {
		notepad.FATAL.Printf("Failed to get site rendering config\n%s", err.Error())
	}

	return combinedConfig
}

var markdownExtensionMap = map[string]int{
	"noIntraEmphasis":        markdown.EXTENSION_NO_INTRA_EMPHASIS,
	"tables":                 markdown.EXTENSION_TABLES,
	"fencedCode":             markdown.EXTENSION_FENCED_CODE,
	"autolink":               markdown.EXTENSION_AUTOLINK,
	"strikethrough":          markdown.EXTENSION_STRIKETHROUGH,
	"laxHtmlBlocks":          markdown.EXTENSION_LAX_HTML_BLOCKS,
	"spaceHeaders":           markdown.EXTENSION_SPACE_HEADERS,
	"hardLineBreak":          markdown.EXTENSION_HARD_LINE_BREAK,
	"tabSizeEight":           markdown.EXTENSION_TAB_SIZE_EIGHT,
	"footnotes":              markdown.EXTENSION_FOOTNOTES,
	"noEmptyLineBeforeBlock": markdown.EXTENSION_NO_EMPTY_LINE_BEFORE_BLOCK,
	"headerIds":              markdown.EXTENSION_HEADER_IDS,
	"titleblock":             markdown.EXTENSION_TITLEBLOCK,
	"autoHeaderIds":          markdown.EXTENSION_AUTO_HEADER_IDS,
	"backslashLineBreak":     markdown.EXTENSION_BACKSLASH_LINE_BREAK,
	"definitionLists":        markdown.EXTENSION_DEFINITION_LISTS,
	"joinLines":              markdown.EXTENSION_JOIN_LINES,
}
...
// getHTMLRenderer creates a new Markdown HTML Renderer with the given configuration.
func (c *ContentSpec) getHTMLRenderer(defaultFlags int, ctx *RenderingContext) markdown.Renderer {
	renderParameters := markdown.HtmlRendererParameters{
	...
	htmlFlags |= markdown.HTML_USE_XHTML
	htmlFlags |= markdown.HTML_FOOTNOTE_RETURN_LINKS

	if ctx.Config.Smartypants {
		htmlFlags |= markdown.HTML_USE_SMARTYPANTS
	}

	if ctx.Config.SmartypantsQuotesNBSP {
		htmlFlags |= markdown.HTML_SMARTYPANTS_QUOTES_NBSP
	}

	if ctx.Config.AngledQuotes {
		htmlFlags |= markdown.HTML_SMARTYPANTS_ANGLED_QUOTES
	}

	if ctx.Config.Fractions {
		htmlFlags |= markdown.HTML_SMARTYPANTS_FRACTIONS
	}

	if ctx.Config.HrefTargetBlank {
		htmlFlags |= markdown.HTML_HREF_TARGET_BLANK
	}

	if ctx.Config.SmartDashes {
		htmlFlags |= markdown.HTML_SMARTYPANTS_DASHES
	}

	if ctx.Config.LatexDashes {
		htmlFlags |= markdown.HTML_SMARTYPANTS_LATEX_DASHES
	}

	return &HugoHTMLRenderer{
		cs:               c,
		RenderingContext: ctx,
		Renderer:         markdown.HtmlRendererWithParameters(htmlFlags, "", "", renderParameters),
	}
}

func getMarkdownExtensions(ctx *RenderingContext) int {
	// Default Markdown common extensions
	commonExtensions := 0 |
		markdown.EXTENSION_NO_INTRA_EMPHASIS |
		markdown.EXTENSION_TABLES |
		markdown.EXTENSION_FENCED_CODE |
		markdown.EXTENSION_AUTOLINK |
		markdown.EXTENSION_STRIKETHROUGH |
		markdown.EXTENSION_SPACE_HEADERS |
		markdown.EXTENSION_HEADER_IDS |
		markdown.EXTENSION_BACKSLASH_LINE_BREAK |
		markdown.EXTENSION_DEFINITION_LISTS

	// Extra Markdown extensions that Hugo enables by default
	flags := commonExtensions |
		markdown.EXTENSION_AUTO_HEADER_IDS |
		markdown.EXTENSION_FOOTNOTES
	...
	for _, extension := range ctx.Config.Extensions {
		if flag, ok := markdownExtensionMap[extension]; ok {
			flags |= flag
		}
	}
...
func (c ContentSpec) markdownRender(ctx *RenderingContext) []byte {
	if ctx.RenderTOC {
		return markdown.Markdown(ctx.Content,
			c.getHTMLRenderer(markdown.HTML_TOC, ctx),
			getMarkdownExtensions(ctx))
	}
	return markdown.Markdown(ctx.Content, c.getHTMLRenderer(0, ctx),
		getMarkdownExtensions(ctx))
}
...
func getAsciidocContent(ctx *RenderingContext) []byte {
	...
	if path == "" {
		...
		if path == "" {
			notepad.ERROR.Println("asciidoctor / asciidoc not found in $PATH: Please install.\n",
	...
	notepad.INFO.Println("Rendering", ctx.DocumentName, "with", path, "...")
...
func getRstContent(ctx *RenderingContext) []byte {
	...
	if path == "" {
		notepad.ERROR.Println("rst2html / rst2html.py not found in $PATH: Please install.\n",
...
func getPandocContent(ctx *RenderingContext) []byte {
	...
	if err != nil {
		notepad.ERROR.Println("pandoc not found in $PATH: Please install.\n",
...
func externallyRenderContent(ctx *RenderingContext, path string, args []string) []byte {
	...
	for _, item := range strings.Split(string(cmderr.Bytes()), "\n") {
		...
		if item != "" {
			notepad.ERROR.Printf("%s: %s", ctx.DocumentName, item)
		}
	}
	if err != nil {
		notepad.ERROR.Printf("%s rendering %s: %v", path, ctx.DocumentName, err)
	}
