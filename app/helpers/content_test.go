...
func TestGetHTMLRendererFlags(t *testing.T) {
	...
	renderer := c.getHTMLRenderer(markdown.HTML_USE_XHTML, ctx)
	flags := renderer.GetFlags()
	if flags&markdown.HTML_USE_XHTML != markdown.HTML_USE_XHTML {
		t.Errorf("Test flag: %d was not found amongs set flags:%d; Result: %d", markdown.HTML_USE_XHTML, flags, flags&markdown.HTML_USE_XHTML)
	}
}
...
func TestGetHTMLRendererAllFlags(t *testing.T) {
	...
	allFlags := []data{
		{markdown.HTML_USE_XHTML},
		{markdown.HTML_FOOTNOTE_RETURN_LINKS},
		{markdown.HTML_USE_SMARTYPANTS},
		{markdown.HTML_SMARTYPANTS_QUOTES_NBSP},
		{markdown.HTML_SMARTYPANTS_ANGLED_QUOTES},
		{markdown.HTML_SMARTYPANTS_FRACTIONS},
		{markdown.HTML_HREF_TARGET_BLANK},
		{markdown.HTML_SMARTYPANTS_DASHES},
		{markdown.HTML_SMARTYPANTS_LATEX_DASHES},
	}
	defaultFlags := markdown.HTML_USE_XHTML
	ctx := &RenderingContext{Cfg: c.cfg, Config: c.NewMarkdown()}
...
func TestGetMarkdownExtensionsMasksAreRemovedFromExtensions(t *testing.T) {
	...
	if actualFlags&markdown.EXTENSION_NO_INTRA_EMPHASIS == markdown.EXTENSION_NO_INTRA_EMPHASIS {
		t.Errorf("Masked out flag {%v} found amongst returned extensions.", markdown.EXTENSION_NO_INTRA_EMPHASIS)
	}
}
...
func TestGetMarkdownExtensionsByDefaultAllExtensionsAreEnabled(t *testing.T) {
	...
	allExtensions := []data{
		...
		// {markdown.EXTENSION_LAX_HTML_BLOCKS},
		{markdown.EXTENSION_SPACE_HEADERS},
		// {markdown.EXTENSION_HARD_LINE_BREAK},
		// {markdown.EXTENSION_TAB_SIZE_EIGHT},
		{markdown.EXTENSION_FOOTNOTES},
		// {markdown.EXTENSION_NO_EMPTY_LINE_BEFORE_BLOCK},
		{markdown.EXTENSION_HEADER_IDS},
		// {markdown.EXTENSION_TITLEBLOCK},
...
func TestGetMarkdownExtensionsAddingFlagsThroughRenderingContext(t *testing.T) {
	...
	actualFlags := getMarkdownExtensions(ctx)
	if actualFlags&markdown.EXTENSION_DEFINITION_LISTS != markdown.EXTENSION_DEFINITION_LISTS {
		t.Errorf("Masked out flag {%v} found amongst returned extensions.", markdown.EXTENSION_DEFINITION_LISTS)
	}
}
...
func TestGetMarkdownRendererWithTOC(t *testing.T) {
	c := newTestContentSpec()
	ctx := &RenderingContext{RenderTOC: true, Cfg: c.cfg, Config: c.NewMarkdown()}
	ctx.Content = []byte("testContent")
	actualRenderedMarkdown := c.markdownRender(ctx)
