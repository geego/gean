...
func TestMakePathSanitized(t *testing.T) {
	v := configurator.New()
	l := NewDefaultLanguage(v)
	p, _ := NewPathSpec(geanfs.NewMem(v), l)
...
func TestDirExists(t *testing.T) {
	...
	for i, d := range data {
		exists, _ := DirExists(filepath.FromSlash(d.input), new(fsintra.OsFs))
...
func TestIsDir(t *testing.T) {
	...
	for i, d := range data {
		...
		exists, _ := IsDir(d.input, new(fsintra.OsFs))
		if d.expected != exists {
			t.Errorf("Test %d failed. Expected %t got %t", i, d.expected, exists)
		}
	}
...
func TestIsEmpty(t *testing.T) {
	...
	for i, d := range data {
		exists, err := IsEmpty(d.input, new(fsintra.OsFs))
...
func TestExists(t *testing.T) {
	...
	for i, d := range data {
		exists, err := Exists(d.input, new(fsintra.OsFs))
		if d.expectedResult != exists {
			t.Errorf("Test %d failed. Expected result %t got %t", i, d.expectedResult, exists)
		}
		if d.expectedErr != err {
...
func TestSafeWriteToDisk(t *testing.T) {
	...
	for i, d := range data {
		e := SafeWriteToDisk(d.filename, reader, new(fsintra.OsFs))
		if d.expectedErr != nil {
...
func TestWriteToDisk(t *testing.T) {
	...
	for i, d := range data {
		e := WriteToDisk(d.filename, reader, new(fsintra.OsFs))
		if d.expectedErr != e {
...
func TestGetTempDir(t *testing.T) {
	...
	for _, test := range tests {
		output := GetTempDir(test.input, new(fsintra.MemMapFs))
		if output != test.expected {
			t.Errorf("Expected %#v, got %#v\n", test.expected, output)
		}
	}
