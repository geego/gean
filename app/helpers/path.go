...
// the file structure.
func SymbolicWalk(fs fsintra.Fs, root string, walker filepath.WalkFunc) error {
	...
	rootContent, err := fsintra.ReadDir(fs, root)
	...
	for _, fi := range rootContent {
		if err := fsintra.Walk(fs, filepath.Join(root, fi.Name()), walker); err != nil {
			return err
		}
	}
...
func getRealFileInfo(fs fsintra.Fs, path string) (os.FileInfo, string, error) {
...
// symlink or not.
func GetRealPath(fs fsintra.Fs, path string) (string, error) {
...
// Code copied from fsintra's path.go
// if the filesystem is OsFs use Lstat, else use fs.Stat
func lstatIfOs(fs fsintra.Fs, path string) (info os.FileInfo, err error) {
	_, ok := fs.(*fsintra.OsFs)
	if ok {
...
func SafeWriteToDisk(inpath string, r io.Reader, fs fsintra.Fs) (err error) {
	return fsintra.SafeWriteReader(fs, inpath, r)
}

// WriteToDisk writes content to disk.
func WriteToDisk(inpath string, r io.Reader, fs fsintra.Fs) (err error) {
	return fsintra.WriteReader(fs, inpath, r)
}

// GetTempDir returns a temporary directory with the given sub path.
func GetTempDir(subPath string, fs fsintra.Fs) string {
	return fsintra.GetTempDir(fs, subPath)
}

// DirExists checks if a path exists and is a directory.
func DirExists(path string, fs fsintra.Fs) (bool, error) {
	return fsintra.DirExists(fs, path)
}

// IsDir checks if a given path is a directory.
func IsDir(path string, fs fsintra.Fs) (bool, error) {
	return fsintra.IsDir(fs, path)
}

// IsEmpty checks if a given path is empty.
func IsEmpty(path string, fs fsintra.Fs) (bool, error) {
	return fsintra.IsEmpty(fs, path)
}

// FileContains checks if a file contains a specified string.
func FileContains(filename string, subslice []byte, fs fsintra.Fs) (bool, error) {
	return fsintra.FileContainsBytes(fs, filename, subslice)
}

// FileContainsAny checks if a file contains any of the specified strings.
func FileContainsAny(filename string, subslices [][]byte, fs fsintra.Fs) (bool, error) {
	return fsintra.FileContainsAnyBytes(fs, filename, subslices)
}

// Exists checks if a file or directory exists.
func Exists(path string, fs fsintra.Fs) (bool, error) {
	return fsintra.Exists(fs, path)
}
