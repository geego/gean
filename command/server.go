...
var serverCmd = &goman.Command{
	Use:     "server",
	Aliases: []string{"serve"},
	Short:   "A high performance webserver",
	Long: `gean provides its own webserver which builds and serves the site.
While gean server is high performance, it is a webserver with limited options.
Many run it in production, but the standard behavior is for people to use it
in development and use a more full featured server such as Nginx or Caddy.

'gean server' will avoid writing the rendered and served content to disk,
preferring to store it in memory.

By default gean will also watch your files for any changes you make and
automatically rebuild the site. It will then live reload any open browser pages
and push the latest content to them. As most gean sites are built in a fraction
...
func server(cmd *goman.Command, args []string) error {
	...
	for i := 0; i < len(serverPorts); i++ {
		...
		if err == nil {
			...
			notepad.ERROR.Println("port", serverPort, "already in use, attempting to use an available port")
	...
	if err := memStats(); err != nil {
		notepad.ERROR.Println("memstats error:", err)
	}
	...
	if !renderToDisk {
		cfg.Fs.Destination = new(fsintra.MemMapFs)
	...
	if serverWatch {
		...
		notepad.FEEDBACK.Printf("Watching for changes in %s%s{%s}\n", baseWatchDir, helpers.FilePathSeparator, rootWatchDirs)
...
func (f *fileServer) createEndpoint(i int) (*http.ServeMux, string, string, error) {
	...
	if i == 0 {
		if renderToDisk {
			notepad.FEEDBACK.Println("Serving pages from " + absPublishDir)
		} else {
			notepad.FEEDBACK.Println("Serving pages from memory")
		}
	}

	httpFs := fsintra.NewHttpFs(f.c.Fs.Destination)
	...
	if i == 0 && fastRenderMode {
		notepad.FEEDBACK.Println("Running in Fast Render Mode. For full rebuilds on change: gean server --disableFastRender")
	}
...
func (c *commandeer) serve() {
	...
	for i, _ := range baseURLs {
		...
		notepad.FEEDBACK.Printf("Web Server is available at %s (bind address %s)\n", serverURL, serverInterface)
		go func() {
			err = http.ListenAndServe(endpoint, mu)
			if err != nil {
				notepad.ERROR.Printf("Error: %s\n", err.Error())
				os.Exit(1)
			}
		}()
	}

	notepad.FEEDBACK.Println("Press Ctrl+C to stop")
}
