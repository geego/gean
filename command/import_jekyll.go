...
var importCmd = &goman.Command{
	Use:   "import",
	Short: "Import your site from others.",
	Long: `Import your site from other web site generators like Jekyll.

Import requires a subcommand, e.g. ` + "`gean import jekyll jekyll_root_path target_path`.",
	RunE: nil,
}

var importJekyllCmd = &goman.Command{
	Use:   "jekyll",
	Short: "gean import from Jekyll",
	Long: `gean import from Jekyll.

Import from Jekyll requires two paths, e.g. ` + "`gean import jekyll jekyll_root_path target_path`.",
...
func importFromJekyll(cmd *goman.Command, args []string) error {

	if len(args) < 2 {
		return newUserError(`Import from Jekyll requires two paths, e.g. ` + "`gean import jekyll jekyll_root_path target_path`.")
	}
	...
	notepad.INFO.Println("Import Jekyll from:", jekyllRoot, "to:", targetDir)
	...
	notepad.FEEDBACK.Println("Importing...")
	...
	for jekyllPostDir, hasAnyPostInDir := range jekyllPostDirs {
		if hasAnyPostInDir {
			if err = helpers.SymbolicWalk(geanfs.Os, filepath.Join(jekyllRoot, jekyllPostDir), callback); err != nil {
				return err
			}
		}
	}

	notepad.FEEDBACK.Println("Congratulations!", fileCount, "post(s) imported!")
	notepad.FEEDBACK.Println("Now, start gean by yourself:\n" +
		"$ git clone https://github.com/spf13/herring-cove.git " + args[1] + "/themes/herring-cove")
	notepad.FEEDBACK.Println("$ cd " + args[1] + "\n$ gean server --theme=herring-cove")
...
func getJekyllDirInfo(fs fsintra.Fs, jekyllRoot string) (map[string]bool, bool) {
...
func retrieveJekyllPostDir(fs fsintra.Fs, dir string) (bool, bool) {
...
func createSiteFromJekyll(jekyllRoot, targetDir string, jekyllPostDirs map[string]bool, force bool) (*geanlib.Site, error) {
	s, err := geanlib.NewSiteDefaultLang()
	if err != nil {
		return nil, err
	}
...
func loadJekyllConfig(fs fsintra.Fs, jekyllRoot string) map[string]interface{} {
	path := filepath.Join(jekyllRoot, "_config.yml")

	exists, err := helpers.Exists(path, fs)

	if err != nil || !exists {
		notepad.WARN.Println("_config.yaml not found: Is the specified Jekyll root correct?")
		return nil
	}
...
func createConfigFromJekyll(fs fsintra.Fs, inpath string, kind string, jekyllConfig map[string]interface{}) (err error) {
...
func convertJekyllPost(s *geanlib.Site, path, relPath, targetDir string, draft bool) error {
	notepad.TRACE.Println("Converting", path)

	filename := filepath.Base(path)
	postDate, postName, err := parseJekyllFilename(filename)
	if err != nil {
		notepad.WARN.Printf("Failed to parse filename '%s': %s. Skipping.", filename, err)
		return nil
	}

	notepad.TRACE.Println(filename, postDate, postName)
	...
	if err != nil {
		notepad.ERROR.Println("Read file error:", path)
		return err
	}

	psr, err := parser.ReadFrom(bytes.NewReader(contentBytes))
	if err != nil {
		notepad.ERROR.Println("Parse file error:", path)
		return err
	}

	metadata, err := psr.Metadata()
	if err != nil {
		notepad.ERROR.Println("Processing file error:", path)
		return err
	}

	newmetadata, err := convertJekyllMetaData(metadata, postName, postDate, draft)
	if err != nil {
		notepad.ERROR.Println("Convert metadata error:", path)
		return err
	}

	notepad.TRACE.Println(newmetadata)
	content := convertJekyllContent(newmetadata, string(psr.Content()))

	page, err := s.NewPage(filename)
	if err != nil {
		notepad.ERROR.Println("New page error", filename)
		return err
	}
	...
	notepad.TRACE.Println("Target file:", targetFile)
...
func convertJekyllMetaData(m interface{}, postName string, postDate time.Time, draft bool) (interface{}, error) {
	...
	metadata, err := assist.ToStringMapE(m)
	if err != nil {
		return nil, err
	}
...
func convertJekyllContent(m interface{}, content string) string {
	metadata, _ := assist.ToStringMapE(m)
